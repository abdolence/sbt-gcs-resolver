...
class GcsRepository(gcsStorage: Storage, bucketName: String, publishPolicy: GcsPublishFilePolicy)(implicit logger: Logger) extends AbstractRepository {
  ...
  override def put(artifact: Artifact, source: File, destination: String, overwrite: Boolean): Unit = {
    Option(gcsStorage.get(bucketName)) match {
      case Some(gcsBucket) => {
        logger.info(s"Publishing GCS artifact to '${destination}'. Artifact: ${artifact.getName}. Overwrite: ${overwrite}")
        GcsRepositoryResource.upload(
          gcsBucket,publishPolicy,source, destination, getContentType(artifact)
        )
      }
      case _ => {
